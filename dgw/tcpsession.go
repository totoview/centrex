...
// TCPSession manages a client connection over TCP.
...
// NewTCPSession creates a new instance of TCPSession.
func NewTCPSession(id uint64, conn net.Conn, stop chan struct{}, done *sync.WaitGroup, logger log.Logger) *TCPSession {
	return &TCPSession{ID: id, conn: conn, stop: stop, done: done, logger: logger}
}

// Start runs TCPSession.
func (s *TCPSession) Start() {

	level.Info(s.logger).Log("op", "start", "tcpsession", s.ID, "from", s.conn.RemoteAddr())
	...
	go func() {
		defer func() { level.Info(s.logger).Log("op", "stop", "tcpsession", s.ID) }()
		defer s.done.Done()
...
func (s *TCPSession) handleLogin(req *pb.Login) {
